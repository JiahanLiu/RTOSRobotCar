...
// **********ST7735 TFT and SDC*******************
// ST7735
// Backlight (pin 10) connected to +3.3 V (Looking at Black Screen - Right side)
...
// Gnd (pin 1) connected to ground (Looking at Black Screen - Left side)
...
// Start Lab 1 Added Defines - Jiahan Liu
#define MIN_DISPLAY0_Y 0
#define MAX_DISPLAY0_Y 6
#define MIN_DISPLAY1_Y 8
#define MAX_DISPLAY1_Y 14
#define MAX_DISPLAY_X 20
#define LINES_PER_DISPLAY 7
#define LINE_OFFSET_DISPLAY1 8
// End Lab 1 Added Defines - Jiahan Liu


/* 
* Lines go from 0 to n-1
*/
//Start Lab 1 Added Vars - Jiahan Liu
uint32_t StX_0 [LINES_PER_DISPLAY] = {0}; // position along the horizonal axis for each line
uint32_t StX_1 [LINES_PER_DISPLAY] = {0}; // position along the horizonal axis for each line
void ST7735_MessageDecBody(uint32_t device, uint32_t line, int32_t value);
void ST7735_MessageStringBody(uint32_t device, uint32_t line, char *string);
// End Lab 1 Added Vars - Jiahan Liu

//Start Lab 2.2 Semaphore Vars - Jiahan Liu
Sema4Type mutexLCD; 
 
// End Lab 2.2 Semaphore Vars - Jiahan Liu
...
void ST7735_OutChar(char ch){
  if((ch == 10) || (ch == 13) || (ch == 27)){ //cariage return?
    StY++; StX=0;
    if(StY>15){
      StY = 0;
    }
...
//Added Lab 1: Jiahan Liu, Eric Li

//------------ST7735_InitRDivided------------
// Initialization for ST7735R screens (green or red tabs).
// Input: option one of the enumerated options depending on tabs
// Output: none
void ST7735_InitRDivided(enum initRFlags option) {	
		commonInit(Rcmd1);
		if(option == INITR_GREENTAB) {
			commandList(Rcmd2green);
			ColStart = 2;
		...
		//Added Lab 1
		ST7735_DrawString(0,7,"---------------------",ST7735_WHITE); //21 long
		OS_InitSemaphore(&mutexLCD, 1); 
}
...
// bgColor is Black and size is 1
// Output: number of characters printed
uint32_t ST7735_DrawStringDivided(uint16_t x, uint16_t y, int device, char *pt, int16_t textColor){
  uint32_t count = 0;
  if(y>LINES_PER_DISPLAY-1) return 0;
	if(device == 0) {
		while(*pt){
			ST7735_DrawCharS(x*6, y*10, *pt, textColor, ST7735_BLACK, 1);
			pt++;
	...
	if(device == 1) {
		while(*pt){
			ST7735_DrawCharS(x*6, (y + LINE_OFFSET_DISPLAY1)*10, *pt, textColor, ST7735_BLACK, 1);
			pt++;
			x = x+1;
			if(x>20) return count;  // number of characters printed
			count++;
...
// Output: none
// Variable format 1-10 digits with no space before or after
// Note: we must avoid sharing globals when possible and we want 
//       each screen to correspond to a thread so they should have 
//       their own variables. 
char MessageDivded0[12];
uint32_t MessageindexDivided0;
char MessageDivded1[12];
uint32_t MessageindexDivided1;

void fillDividedMessage(uint32_t n, uint32_t device){
// This function uses recursion to convert decimal number
//   of unspecified length as an ASCII string
	if(device == 0) {
		if(n >= 10){
			fillDividedMessage(n/10, device);
			n = n%10;
		}
		MessageDivded0[MessageindexDivided0] = (n+'0'); /* n is between 0 and 9 */
		if(MessageindexDivided0<11)MessageindexDivided0++;
	}
	...
	if(device == 1) {
		...
		MessageDivded1[MessageindexDivided1] = (n+'0'); /* n is between 0 and 9 */
		if(MessageindexDivided1<11)MessageindexDivided1++;
	}
}

// *************** ST7735_OutCharDivided ********************
// Output one character to the LCD
// Position determined by ST7735_SetCursor command
// Color set by ST7735_SetTextColor
// Inputs: 8-bit ASCII character
//         device: 0 = top, 1 = bottom
//         line: line number from 0 to 8
// Outputs: none
void ST7735_OutCharDivided(char ch, uint32_t line, uint32_t device){
	int lineIndex = line;
	if(device == 0) {
		if((ch == 10) || (ch == 13) || (ch == 27)){ //cariage return or something similar?
			ST7735_DrawStringDivided(0,line, 0,"                     ",StTextColor);
			return;
		}
		ST7735_DrawCharS(StX_0[line]*6,line*10,ch,ST7735_YELLOW,ST7735_BLACK, 1); 
		StX_0[lineIndex]++;
		if(StX_0[lineIndex]>MAX_DISPLAY_X){
			StX_0[lineIndex] = MAX_DISPLAY_X;
			ST7735_DrawCharS(StX_0[lineIndex]*6,line*10,'*',ST7735_RED,ST7735_BLACK, 1);
		}
		return;
	}
	if(device == 1) {
		if((ch == 10) || (ch == 13) || (ch == 27)){ //cariage return?
			ST7735_DrawStringDivided(0,line, 1 ,"                     ",StTextColor);
			return;
		}
		ST7735_DrawCharS(StX_1[lineIndex]*6,(line + LINE_OFFSET_DISPLAY1)*10,ch,ST7735_YELLOW,ST7735_BLACK, 1);
		StX_1[lineIndex]++;
		if(StX_1[lineIndex]>MAX_DISPLAY_X){
			StX_1[lineIndex] = MAX_DISPLAY_X;
			ST7735_DrawCharS(StX_1[lineIndex]*6,(line + LINE_OFFSET_DISPLAY1)*10,'*',ST7735_RED,ST7735_BLACK, 1);
		}
		return;
	}
}

//********ST7735_SetCursorDivided*****************
// SetCursor For Divided Screen
void ST7735_SetCursorDivided(uint32_t device, uint32_t line, uint32_t newX){
  if((newX > 20) || (line > LINES_PER_DISPLAY-1)){       // bad input
    return;                             // do nothing
  }
	if(device == 0) {
		StX_0[line] = newX;
...
/*********ST7735_MessageDec*****************
Prints String to chosen screens segment of screen.
intputs: 	device: 0 = top, 1 = bottom
					line: 8 lines per display segment
					value: int input
*/
...
// Position determined by ST7735_SetCursor command
// Color set by ST7735_SetTextColor
// Inputs: 32-bit number to be transferred
//         device: 0 = top, 1 = bottom
//         line: line number from 0 to 8
// Output: none
// Variable format 1-10 digits with no space before or after
void ST7735_MessageDec(uint32_t device, uint32_t line, int32_t value) {
	OS_Wait(&mutexLCD); 
	ST7735_DrawStringDivided(0,line, device, "                     ",StTextColor);  //clear line
	ST7735_SetCursorDivided(device,line,0);
	ST7735_MessageDecBody(device,line,value);
	OS_Signal(&mutexLCD); 
}
...
void ST7735_MessageDecBody(uint32_t device, uint32_t line, int32_t value) {
	if(line < LINES_PER_DISPLAY) {
		int lineIndex = line; 
		if(device == 0) {
			MessageindexDivided0 = 0;
			fillDividedMessage(value, device);
			MessageDivded0[MessageindexDivided0] = 0; // terminate
			ST7735_DrawStringDivided(StX_0[lineIndex],line, device, MessageDivded0,StTextColor);
			StX_0[lineIndex] = StX_0[lineIndex]+MessageindexDivided0;
			if(StX_0[lineIndex]>MAX_DISPLAY_X){
				StX_0[lineIndex] = MAX_DISPLAY_X;
				ST7735_DrawCharS(StX_0[lineIndex]*6,line*10,'*',ST7735_RED,ST7735_BLACK, 1);
			}
		...
		if(device == 1) {
			MessageindexDivided1 = 0;
			fillDividedMessage(value, device);
			MessageDivded1[MessageindexDivided1] = 0; // terminate
			ST7735_DrawStringDivided(StX_1[lineIndex],line, device ,MessageDivded1,StTextColor);
			StX_1[lineIndex] = StX_1[lineIndex]+MessageindexDivided1;
			if(StX_1[lineIndex]>MAX_DISPLAY_X){
				StX_1[lineIndex] = MAX_DISPLAY_X;
				ST7735_DrawCharS(StX_1[lineIndex]*6,(line+ LINE_OFFSET_DISPLAY1)*10,'*',ST7735_RED,ST7735_BLACK, 1);
			}
...
Prints String to chosen displays segment of screen.
intputs: 	device: 0 = top, 1 = bottom
					line: 8 lines per display segment
					value: int input
*/
void ST7735_MessageString(uint32_t device, uint32_t line, char *string) {
	OS_Wait(&mutexLCD); 
	ST7735_DrawStringDivided(0,line, device, "                     ",StTextColor);  //clear line
	ST7735_SetCursorDivided(device,line,0);
	ST7735_MessageStringBody(device, line, string); 
	OS_Signal(&mutexLCD); 
}
...
void ST7735_MessageStringBody(uint32_t device, uint32_t line, char *string) {
	if(line < LINES_PER_DISPLAY) {
		while(*string){
			ST7735_OutCharDivided(*string, line, device);
			string = string + 1;
		}
	}
}

void ST7735_Message(uint32_t device, uint32_t line, char *string, int32_t value) {
	OS_Wait(&mutexLCD); 
	ST7735_DrawStringDivided(0,line,device,"                     ",StTextColor);  //clear line
	ST7735_SetCursorDivided(device,line,0);
	//--- Message String ---
