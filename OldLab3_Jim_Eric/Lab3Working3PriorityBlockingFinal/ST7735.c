...
void ST7735_OutCharDivided(char ch, uint32_t line, uint32_t device){
	...
	if(device == 0) {
		if((ch == 10) || (ch == 13) || (ch == 27)){ //cariage return or something similar?
			ST7735_DrawString(0,line,"                     ",StTextColor);
			return;
		}
	...
	if(device == 1) {
		if((ch == 10) || (ch == 13) || (ch == 27)){ //cariage return?
			ST7735_DrawString(0,line + LINE_OFFSET_DISPLAY1,"                     ",StTextColor);
			return;
		}
...
void ST7735_MessageDec(uint32_t device, uint32_t line, int32_t value) {
	...
	if(device == 0) {
		...
		ST7735_DrawString(StX_0[lineIndex],line,MessageDivded0,StTextColor);
	...
	if(device == 1) {
		...
		ST7735_DrawString(StX_1[lineIndex],(line + LINE_OFFSET_DISPLAY1),MessageDivded1,StTextColor);
