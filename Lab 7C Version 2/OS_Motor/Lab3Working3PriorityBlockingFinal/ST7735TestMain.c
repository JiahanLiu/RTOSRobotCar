...
void straight(int pServo, int pLeft, int pRight) {
	...
	Right_Duty(12500-PowerRight,1);  // 12400 to 400 (negative logic)
	
	PowerLeft = POWERMIN + pLeft * POWERDELTA;
	Left_Duty(PowerLeft,0);       // 400 to 12400 (positive logic)
}
...
void HardRight(int pServo, int pLeft, int pRight) {
	...
	//Steering = SERVOMID - pServo * SERVODELTA;
	Steering = SERVOMIN + SERVODELTA;
	Servo_Duty(Steering);    // SERVOMIN to SERVOMAX
	
	//PowerRight = POWERMIN + pRight * POWERDELTA;
	PowerRight = POWERMAX-(0.75*POWERMAX);
	Right_Duty(PowerRight, 1);
	Right_DutyB(PowerRight,1);  // 12400 to 400 (negative logic)
	
	//PowerLeft = POWERMIN + pLeft * POWERDELTA;
	PowerLeft = 0.2*POWERMAX;
	Left_Duty(PowerLeft,0);       // 400 to 12400 (positive logic)
	Left_DutyB(PowerLeft,0);
}
...
void HardLeft(int pServo, int pLeft, int pRight) {
	...
	//PowerRight = POWERMIN + pRight * POWERDELTA;
	PowerRight = 0.5*POWERMAX;
	Right_Duty(PowerRight, 0);
	Right_DutyB(PowerRight,0);  // 12400 to 400 (negative logic)
	
	//PowerLeft = POWERMIN + pLeft * POWERDELTA;
	PowerLeft = 0.8*POWERMAX;
	Left_Duty(PowerLeft,0);       // 400 to 12400 (positive logic)
	Left_DutyB(PowerLeft,0);
}
...
int mainz(void){
  ...
  OS_Fifo_Init(32);    // ***note*** 4 is not big enough*****
	...
	numCreated += OS_AddThread(&Leon,128,2);  // Lab 3, make this lowest priority
